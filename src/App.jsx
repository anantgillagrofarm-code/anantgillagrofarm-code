...
/*
  Images expected in /public:
   ...
   - /footer-mushrooms-v2.jpg   <-- NEW NAME (important)
   - /anant_gill_logo.png
*/
...
export default function App() {
  ...
  // Disable body scroll when sheet or drawer open
  useEffect(() => {
  ...
  const totals = cart.reduce((acc, it) => {
    ...
    const v = p && p.variants.find((vv) => vv.id === it.variantId);
  ...
  useEffect(() => setMiniCartVisible(totals.items > 0), [totals.items]);
  ...
  function addToCartImmediate(product, variantId = null, qty = 1) {
    ...
    setCart((prev) => {
      ...
      if (existing) {
        return prev.map((i) => i.productId === product.id && i.variantId === vId ? { ...i, qty: i.qty + qty } : i);
      } else {
  ...
  return (
    <div className="app-root">
      <header className="site-header">
        ...
        <button className="cart-button" onClick={() => setCartOpen(true)}>Cart {totals.items > 0 ? `(${totals.items})` : ""}</button>
      </header>
          ...
          <div className="product-grid">
            {PRODUCTS.map(p => (
              <article className="card" key={p.id}>
                <div className="card-image"><img src={p.image} alt={p.title} /></div>
                <div className="card-body">
                  ...
                  <div className="price-row">
                    ...
                    <div className="actions">
                      <button className="btn btn-primary" onClick={() => handleAddClick(p)}>Add to Cart</button>
                      <button className="btn btn-ghost" onClick={() => { setSelectedVariant(p.variants[0].id); setSheetProduct(p); }}>Details</button>
                    </div>
                  </div>
                </div>
              </article>
        ...
        <aside className="cart-card">
          ...
          {cart.length === 0 ? <p className="muted">Your cart is empty</p> : (
            <>
              {cart.map(it => {
                ...
                return (
                  <div key={`${it.productId}-${it.variantId}`} className="cart-item">
                    <div><strong>{product.title}</strong> × {it.qty}</div>
              ...
              <div className="subtotal">Subtotal<br/>{formatRupee(totals.subtotal)}</div>
            </>
          )}
        </aside>
      </main>
          ...
          <div className="sheet" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="variants">
              ...
              {sheetProduct.variants.map(v => (
                <label key={v.id} className={`variant-card ${selectedVariant === v.id ? "selected" : ""}`}>
                  <input type="radio" name="variant" checked={selectedVariant === v.id} onChange={() => setSelectedVariant(v.id)} />
                  <div className="variant-info"><div className="variant-name">{v.name}</div><div className="variant-price">{formatRupee(v.price)}</div></div>
                </label>
              ))}
            </div>

            <div className="sheet-actions">
              <button className="btn btn-primary" onClick={confirmAddFromSheet}>Add to Cart</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className={`cart-drawer ${cartOpen ? "open" : ""}`}>
        <div className="drawer-header"><h3>Cart</h3><button className="drawer-close" onClick={() => setCartOpen(false)}>✕</button></div>
        <div className="drawer-body">
          {cart.length === 0 ? <p className="muted">Your cart is empty</p> : (
            <>
              {cart.map(it => {
                ...
                return (
                  <div key={`${it.productId}-${it.variantId}`} className="drawer-item">
                    <div><strong>{product.title}</strong> × {it.qty}<div className="muted">{variant.name}</div></div>
              ...
              <div className="drawer-subtotal">Subtotal<br/>{formatRupee(totals.subtotal)}</div>
            </>
          )}
        </div>
      </div>
      ...
      {/* Mini-bar */}
      {miniCartVisible && totals.items > 0 && (
        <div className="mini-cart">
          <div className="mini-left">
            <div><strong>{totals.items} item</strong></div>
            <div className="muted">Subtotal {formatRupee(totals.subtotal)}</div>
          </div>
          <div className="mini-right">
            <button className="btn btn-primary" onClick={() => setCartOpen(true)}>View Cart</button>
          </div>
        </div>
      )}

      {/* Footer uses footer-mushrooms-v2.jpg to force new asset */}
